#ifndef __MULTIKEY_STORAGE__H
#define __MULTIKEY_STORAGE__H
...
{
	class MultikeyStorage : public IStreamStorage
	{
	public:
		MultikeyStorage(std::streambuf& _index_buf, std::streambuf& _data_buf) : m_indexOut(&_index_buf),
		...
		virtual ~MultikeyStorage() override
		{
		}
	...
	private:
		...
		*   @return: true if bucket is vacant, otherwise - false*/
		...
		//! Get last data record recursively bypassing all records begining with record under '_offset'
		bool getLastData(const int64_t _offset, DataRecord& _outData, int64_t& _outOffset) const;
		...
		*   @return: true if record with required key found, otherwise false and out_data and out_offset will be initialized with last record*/
		...
		//! Delete record
		void DeleteRecord(const DataRecord& _deletedRecord,
					const int64_t deletedOffset,
					const size_t index);

		//! Delete all record with appropriate '_key' and 'predicate' test
